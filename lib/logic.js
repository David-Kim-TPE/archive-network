...
/**
 ...
 async function demo(demo) {
  ...
  const secretary = factory.newRelationship(namespace, 'Secretary', 'AliceTsao');
  odModel.forEach((od)=> {
    console.log({od});
    const odr = factory.newResource(namespace, 'OfficialDocument', od.Issue_No);
    odr['Creator'] = secretary;
...
async function archive(archive) {
  ...
  archive.od.End_File_Submiter = currentParticipant ;
  archive.od.End_File_URL = archive.End_File_URL;
  archive.od.File_URL = "";
  ...
  const archiveEvent = factory.newEvent(namespace, 'ArchiveEvent');
...
async function archivePrint(archivePrint) {
  ...
  archivePrint.od.End_File_Printer = secretary;
  ...
  const archivePrintEvent = factory.newEvent(namespace, 'ArchivePrintEvent');
